...
function LiveDocTab({ doc, blueprint, onEmbed }) {
  const [embedState, setEmbedState] = useState("idle");
  if (!doc) {
    return (
      <div className="p-6">
  ...
  return (
    <>
      ...
      {blueprint && blueprint.url && (
        <LiveBlueprintCard blueprint={blueprint} />
      )}
    </>
  );
}

// ── Migration Blueprint download card (live flow, no embed) ──────────────────
//
// Mirrors DemoBlueprintCard's visual treatment but for the dynamic blueprint
// generated by Pass 3. No embed action — the blueprint is implementation
// guidance, not searchable Q&A content.
function LiveBlueprintCard({ blueprint }) {
  const filename = blueprint.url
    ? blueprint.url.split("/").pop()
    : `${blueprint.title || "migration_blueprint"}.docx`;
  const subtitle = blueprint.title
    ? `${blueprint.title} — How to bridge the gap to standard QAD Adaptive`
    : "Migration Blueprint — implementation steps, code, and configuration";
  ...
  return (
    <div className="px-6 pb-2">
      <div className="flex items-center justify-between gap-4" style={cardDark}>
        ...
        <a href={blueprint.url} download={filename}
          className="shrink-0 px-4 py-2 text-xs font-semibold rounded-lg transition flex items-center gap-1.5"
          style={{ background: "linear-gradient(135deg,#6366f1,#818cf8)", color: "#0b1130" }}>
          <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24"
            fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round">
...
function AnalysisPanel({ summary, doc, blueprint, onEmbed }) {
  ...
  return (
    <TabbedShell
      ...
      docNode={<LiveDocTab doc={doc} blueprint={blueprint} onEmbed={onEmbed} />}
    />
  );
}
...
export function QadZone() {
  ...
  const sendChat = useCallback((overrideQuestion) => {
    ...
    let extraDoc = null, extraFollowups = null, extraSummary = null, extraBlueprint = null;
    ...
    const closeWs = openWs({ mode, question, uploaded_files: filesToSend }, {
      ...
      onFrame:  (type, data) => {
        ...
        if (type === "blueprint") extraBlueprint = data;
      },
      onDone: (acc) => {
        ...
        // backend frames arrived (graceful degradation). The blueprint is
        // optional — it ships only when Pass 3 succeeds.
        if (isDocMode && (extraSummary || extraDoc)) {
          msg.isAnalysis = true;
          msg.summary    = extraSummary;
          msg.blueprint  = extraBlueprint;
        }
